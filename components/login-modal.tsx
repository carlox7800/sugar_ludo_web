...
import { X, Code2, Loader2, AlertCircle } from 'lucide-react'
...
export function LoginModal({ isOpen, onClose, onLoginGoogle, onLoginDev }: LoginModalProps) {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      console.error('[LoginModal] Fallo en inicio de sesión:', err)
      const message = err?.message || (typeof err === 'string' ? err : JSON.stringify(err))
      setErrorMessage(message || 'Error desconocido al autenticar.')
    } finally {
      setIsLoading(false)
    }
  }
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
      ...
      <div className="animate-in fade-in zoom-in-95 glass relative z-10 flex w-full max-w-sm flex-col overflow-hidden rounded-3xl border border-[var(--candy-cyan)]/30 p-8 text-center shadow-[0_0_40px_oklch(0.7_0.27_350/0.15)]">
        ...
        {errorMessage && (
          <div className="mb-6 flex items-start gap-2.5 rounded-xl border border-red-500/40 bg-red-500/10 p-3 text-left animate-in fade-in zoom-in-95">
            <AlertCircle className="size-5 shrink-0 text-red-400 mt-0.5" />
            <div className="flex-1">
              <span className="block text-xs font-bold text-red-300 uppercase tracking-wider">Aviso de Diagnóstico</span>
              <p className="text-xs text-red-200 mt-0.5 break-words font-mono leading-relaxed">{errorMessage}</p>
            </div>
          </div>
        )}
        ...
        <div className="flex flex-col gap-4">
          <button
            ...
            disabled={isLoading}
            className="btn-3d flex items-center justify-center gap-3 rounded-xl bg-white px-4 py-3.5 text-sm font-bold text-black shadow-[0_4px_12px_rgba(255,255,255,0.2)] hover:bg-gray-100 disabled:opacity-75 disabled:cursor-not-allowed transition-all"
          >
            {isLoading ? (
              <>
                <Loader2 className="size-5 animate-spin text-[var(--candy-magenta)]" />
          ...
          <button
            onClick={onLoginDev}
            disabled={isLoading}
            className="flex items-center justify-center gap-2 rounded-xl border border-[var(--candy-cyan)]/30 bg-[var(--candy-cyan)]/10 px-4 py-3 text-sm font-bold text-[var(--candy-cyan)] hover:bg-[var(--candy-cyan)]/20 transition-colors disabled:opacity-50"
          >

...
code = code.replace(
  /<div className="flex gap-3">[\s\S]*?\{!hasRolled && !isRolling \? \(/,
  `<div className="flex justify-center w-full my-4">
              <div
                onClick={() => isHumanTurnToRoll && !isRolling && !hasRolled && onRollDice()}
                className={\`w-20 h-20 rounded-2xl flex items-center justify-center transition-all duration-300 relative \${
              ...
              >
                {diceValue !== null ? (
                  ...
                  <span className="text-white/60 font-bold text-3xl font-mono">?</span>
                )}
              </div>
            </div>
          </div>
